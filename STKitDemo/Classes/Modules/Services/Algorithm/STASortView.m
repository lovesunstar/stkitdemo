...
- (void)reloadSortDataSource:(NSArray *)dataSource {
    [self.elementViewArray enumerateObjectsUsingBlock:^(UIView *subView, NSUInteger idx, BOOL *stop) { [subView removeFromSuperview]; }];
...
- (void)moveBaseline1ToIndex:(NSInteger)index duration:(NSTimeInterval)duration completion:(void (^)(BOOL finished))completion {
    CGFloat left = [self frameXAtIndex:index];
    CGRect frame = self.baseline1.frame;
    frame.origin.x = left;
    [UIView animateWithDuration:duration
        animations:^{ self.baseline1.frame = frame; }
...
- (void)moveBaseline2ToIndex:(NSInteger)index duration:(NSTimeInterval)duration completion:(void (^)(BOOL finished))completion {
    CGFloat left = [self frameXAtIndex:index];
    CGRect frame = self.baseline2.frame;
    frame.origin.x = left;
    [UIView animateWithDuration:duration
        animations:^{ self.baseline2.frame = frame; }
        completion:^(BOOL finished) {
...
- (void)moveElementAtIndex:(NSInteger)index
    ...
    [UIView animateWithDuration:duration
        animations:^{ subview.frame = frame; }
        completion:^(BOOL finished) {
            subview.frame = frame;
            if (completion) {
                completion(finished);
...
- (void)moveElementToCacheAtIndex:(NSInteger)index duration:(NSTimeInterval)duration completion:(void (^)(BOOL finished))completion {
    UIView *subview = [self viewAtIndex:index];
    CGRect frame = subview.frame;
    frame.origin.y = 0;
...
- (void)removeElementFromCacheWithDuration:(NSTimeInterval)duration completion:(void (^)(BOOL finished))completion {
    ...
    [UIView animateWithDuration:duration
        animations:^{ self.cacheView.frame = frame; }
        completion:^(BOOL finished) {
